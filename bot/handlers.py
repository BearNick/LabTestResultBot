...
# Парсеры: сначала Vision, потом текстовый фолбэк
...
async def handle_pdf(message: Message, state: FSMContext):
    ...
    try:
        ...
        # 2) Парсинг: VISION → фолбэк текстовый
        ...
        # 5) Оплата: создаём инвойс CryptoBot (если токен задан), плюс DonationAlerts
        crypto_url = None
        try:
        ...
        # 6) Меню оплаты (RUB + USDT + «Я оплатил»)
